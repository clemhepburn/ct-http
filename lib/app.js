...
const app = net.createServer(socket => {
  socket.on('data', data => {
    ...
    if(request.method === 'GET' && request.path === '/') {
      ...
      socket.write(createResponse({ body: '<h1>red</h1>', contentType: 'text/plain' }));

    } else if(request.method === 'GET' && request.path === '/green') {
      socket.write(createResponse({ body: '<h1>green</h1>', contentType: 'text/plain' }));      
    } else if(request.method === 'GET' && request.path === '/blue') {
      socket.write(createResponse({ body: '<h1>blue</h1>', contentType: 'text/plain' }));
